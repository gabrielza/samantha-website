import { Link } from 'react-router-dom';
import contact from '../../data/contact';
import jp from '../../data/justproperty';

export default function Footer() {
  return (
    <footer className="bg-navy-800">
      {/* Main */}
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {/* Brand */}
          <div>
            <Link to="/" className="inline-block">
              <span className="text-lg font-semibold text-white">Samantha Black</span>
              <span className="block text-[10px] uppercase tracking-[0.2em] text-gold-400 mt-0.5">
                Just Property Cape Town
              </span>
            </Link>
            <p className="mt-4 text-[13px] text-gray-400 leading-relaxed">
              Property Practitioner with over {contact.yearsExperience} years of expertise
              across the Atlantic Seaboard, City Bowl, and Northern Suburbs.
            </p>
            <div className="flex gap-3 mt-5">
              <a href={contact.social.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="h-9 w-9 rounded-lg bg-navy-700 flex items-center justify-center text-gray-400 hover:text-gold-400 hover:bg-navy-600 transition-all">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
              </a>
              <a href={contact.social.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="h-9 w-9 rounded-lg bg-navy-700 flex items-center justify-center text-gray-400 hover:text-gold-400 hover:bg-navy-600 transition-all">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
              </a>
            </div>
          </div>

          {/* Explore */}
          <div>
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.15em] text-gold-400 mb-4">
              Explore
            </h4>
            <ul className="space-y-2.5">
              {[
                { to: '/properties', label: 'Properties' },
                { to: '/neighborhoods', label: 'Neighborhoods' },
                { to: '/about', label: 'About Samantha' },
                { to: '/resources', label: 'Resources & Tools' },
                { to: '/cape-town-day-out', label: 'Cape Town Day Out' },
                { to: '/contact', label: 'Contact' },
              ].map((link) => (
                <li key={link.to}>
                  <Link to={link.to} className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Property Search */}
          <div>
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.15em] text-gold-400 mb-4">
              Property Search
            </h4>
            <ul className="space-y-2.5">
              <li><a href={jp.forSale()} target="_blank" rel="noopener noreferrer" className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">Residential For Sale</a></li>
              <li><a href={jp.toRent()} target="_blank" rel="noopener noreferrer" className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">Residential To Let</a></li>
              <li><a href={jp.freeValuation} target="_blank" rel="noopener noreferrer" className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">Free Property Valuation</a></li>
              <li><a href={jp.calculators} target="_blank" rel="noopener noreferrer" className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">Bond Calculators</a></li>
              <li><a href={jp.emailAlerts} target="_blank" rel="noopener noreferrer" className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">Property Email Alerts</a></li>
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="text-[11px] font-semibold uppercase tracking-[0.15em] text-gold-400 mb-4">
              Get in Touch
            </h4>
            <ul className="space-y-2.5">
              <li>
                <a href={`tel:${contact.phoneRaw}`} className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">
                  {contact.phone}
                </a>
              </li>
              <li>
                <a href={`mailto:${contact.email}`} className="text-[13px] text-gray-400 hover:text-gold-400 transition-colors">
                  {contact.email}
                </a>
              </li>
              <li>
                <a
                  href={`https://wa.me/${contact.whatsapp}?text=${encodeURIComponent(contact.whatsappMessage)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-[13px] text-gray-400 hover:text-green-400 transition-colors"
                >
                  <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/></svg>
                  WhatsApp
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-navy-700/50">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-[11px] text-gray-500">
            <p>&copy; {new Date().getFullYear()} Samantha Black — Just Property Cape Town</p>
            <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
              <span>PPRA Registered</span>
              <span className="text-navy-600">·</span>
              <span>eDRS Compliant</span>
              <span className="text-navy-600">·</span>
              <span>Property Practitioner</span>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
